...
export const getLogsByDate = async (req, res) => {
  try {
    const logs = await DailyLog.getByDate(req.params.date);
    res.json({ success: true, data: logs });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
  }
...
export const createOrUpdateLog = async (req, res) => {
  try {
    ...
    const log = await DailyLog.findOneAndUpdate(
      { date: logDate, habitId },
      { completed, value, note, mood, date: logDate },
...
export const bulkCreateOrUpdateLogs = async (req, res) => {
  try {
    ...
    const operations = logs.map(log => {
      ...
      return {
        updateOne: {
          filter: { date: logDate, habitId: log.habitId },
          update: {
    ...
    for (const key of affectedMonths) {
      ...
      await MonthlySummary.recalculate(habitId, parseInt(year), parseInt(month));
    }
...
export const saveReflection = async (req, res) => {
  try {
    ...
    let log = await DailyLog.findOne({ date: logDate });
